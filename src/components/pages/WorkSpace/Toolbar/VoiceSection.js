...
import { ChevronDown, DropDownIcon, PenIcon, PlayCircleIcon, PlusIcon, TimesIcon, VoiceIcon, VoiceLine } from "../../../Svg";
    ...
    return (
        <div>
            <div className="flex items-start justify-between p-3 gap-3 border-b border-[#262626]" >
                <div className="px-4 py-1.5 h-10 bg-[#161616] border border-[#404040] cursor-pointer rounded-[4px] w-full flex items-center gap-6 hover:bg-[#404040] " onClick={() => setVoiceOpen(true)}>
                    <PlusIcon />
                    <label className="text-white uppercase text-xs tracking-[.21rem] cursor-pointer">Add New</label>
                </div>
                {
                    isVoiceOpen && (
                        <div
                            className="drop-shadow-[0_15px_15px_rgba(255,255,255,0.2)] menu menu-compact  shadow bg-[#2B2B2B] border border-[#161616] rounded-[4px] w-[400px] absolute right-[25rem]"
                        >
                            ...
                            <div className="flex h-20 items-center justify-center px-[24px]  w-full">
                                {
                                    voiceState === 0 &&
                                    <div className="w-16 h-16 rounded-full cursor-pointer flex justify-center items-center" style={{ background: " linear-gradient(90deg, #FEAC6D 0%, #AE61ED 100%)" }} onClick={() => { RecordingStart() }} >
                                        <VoiceIcon />
                                    </div>
                                }
                                {
                                    ...
                                    <div className="flex w-16 h-16" onClick={() => RecordingEnd()}>
                                        <div className="animate-ping absolute w-16 h-16 rounded-full opacity-40 cursor-pointer" style={{ background: " linear-gradient(90deg, #FEAC6D 0%, #AE61ED 100%)" }} />
                                        <div className="flex items-center justify-center rounded-full h-16 w-16 " style={{ background: " linear-gradient(90deg, #FEAC6D 0%, #AE61ED 100%)" }} >
                                ...
                                }
                                {
                                    ...
                                    <div className="flex justify-end w-full gap-4">
                                        <button className=" w-28 h-10 border border-[#404040] rounded tracking-widest text-white uppercase">Delete</button>
                                        <button className=" w-28 h-10 border border-[#404040] rounded tracking-widest text-white uppercase" style={{ background: "linear-gradient(90deg, #FEAC6D 0%, #AE61ED 100%)" }} onClick={FinishVoice}>Save</button>
                                    </div>
                                }
                            </div>
                        </div>
            ...
            <div className="max-h-[calc(100vh-220px)] overflow-auto CustomScroll">
                <div className="flex items-start flex-col justify-between p-3 gap-3 border-b border-[#262626]" >
                    {
                        [1, 2, 3, 4, 5].map((index) => (
                            <div className="border border-[#2B2B2B]  flex flex-col p-4 gap-4 w-full rounded" key={index}>
                                <div className="flex justify-between w-full">
                                    <div className="flex items-center justify-between gap-2 w-full">
                                        <label className="text-white uppercase text-xs tracking-[.21rem]">Voice Memo Name </label>
                                        <div className="text-white uppercase text-xs tracking-[.21rem] w-5 h-5 hover:bg-[#404040] rounded flex items-center justify-center"><PenIcon /></div>
                                    </div>
                                </div>
                                <div className="w-full h-fit flex  justify-between items-center">

                                    <div className={index % 2 ? "text-[#1DAEFF]" : " " + "w-14 h-14  flex"}>
                                        <CgPlayButtonO size={"50"} />
                                    </div>
                                    <div>
                                        {
                                            index % 2 ?
                                                <img src={process.env.PUBLIC_URL + "/assets/img/Workspace/VoiceLineLight.png"} /> :
                                                <img src={process.env.PUBLIC_URL + "/assets/img/Workspace/VoiceLineDark.png"} />
                                        }
                                    </div>
                                    <div className=" border border-[#2b2b2b] bg-[#0E0E0E] rounded-2xl h-8 w-16 flex items-center justify-center">
                                        <label className="text-xs uppercase tracking-[.21rem] text-white">1:45</label>
                                    </div>
                                </div>
                                <div className="flex justify-between">
                                    <div className="flex items-center justify-center gap-2">
                                        <label className="text-xs uppercase tracking-[.21rem]">Irina</label>
                                    </div>
                                    <div className="flex justify-between gap-2">
                                        <label className="uppercase text-xs tracking-[.21rem]">2 Days Ago</label>
                                    </div>
                                </div>
                            </div>
                        ))
